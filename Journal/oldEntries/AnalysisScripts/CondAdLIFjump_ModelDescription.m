...
for ss = 1:length(sigvals)
    ...
    [sigmatest.SimValues(ss)] = AdLIFfunction_jump(PopParams,TimeParams,'showfig',false);
end
...
for ss = 1:length(thetavals)
    ...
    [thetatest.SimValues(ss)] = AdLIFfunction_jump(PopParams,TimeParams,'showfig',false);
end
...
PopParams.E_e     = 0;      %rev potential: E (mV)
PopParams.E_i     = -80;    %rev potential: I (mV)
tau_s     = [1 2 5 2 2 2];      %synaptic decay timescale (ms)
...
for ii = 1:6
    ...
    PopParams.tau_s = tau_s(ii);
    [testvals(ii)] = AdLIFfunction_jump(PopParams,TimeParams,...
...
figure
    ...
    subplot(4,2,7)
        ...
        legend(num2str(tau_s(3)),num2str(tau_s(2)),num2str(tau_s(1)))
        xlim(viewwin)
        ylabel('s_p_r_e')
...
PopParams.E_e     = 0;      %rev potential: E (mV)
PopParams.E_i     = -80;    %rev potential: I (mV)
tau_s     = [0.5 0.2 0.1 1 1 1];      %synaptic decay timescale (1/ms)
