function CondAdLIF_iSTDP_Adaptation_Current(cluster_number)
...
% load([pathtofile 'Lognormal_m_1_s_10_EE_UniformRates_Noise_10ms_50pA_K_IE_250.mat']);
% load([pathtofile 'Lognormal_m_1_s_10_EE_UniformRates_Noise_10ms_50pA_K_IE_250_starting_values.mat']);

load([pathtofile 'Lognormal_m_1_s_10_EE_LognormalRates_Noise_10ms_50pA_K_IE_250.mat']);
load([pathtofile 'Lognormal_m_1_s_10_EE_LognormalRates_Noise_10ms_50pA_K_IE_250_initial_values.mat']);
%load([pathtofile 'Lognormal_m_1_s_10_EE_LognormalRates_Noise_10ms_50pA_K_IE_250_starting_values.mat']);
...
PopParamsAnalysis.t_syn        = 0;                      %Synaptic Delay (ms)
...
UnRecordedTime = 3e4;                   %Unrecorded time
RecordTime = 2e4;                       %Recording Time (end of simulation)
SimTime  = UnRecordedTime+RecordTime;   %Total Simulation time

TimeParams.SimTime = SimTime;

Ivals = linspace(150,300,20); %Current Values (pA)
...
for II = 1:(length(Ivals)*length(bvals))
    ...
    save(['AdaptationData/AdaptationVCurrentSpikes_ii_' num2str(ii) '_bb_' num2str(bb) '.mat'],'spikes','-v7.3') 
