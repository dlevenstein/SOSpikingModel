...
%--------------------------------------------------------------------------
function [SimValues] = CAdLIFfunction_STDP(PopParams,TimeParams)
...
save_weights = 10;
cellout = true;
recordIntervals = [0,TimeParams.SimTime-1000;1000,TimeParams.SimTime];
...
Ecells = 1:EPopNum;             EcellIDX = 1:PopNum <= EPopNum; %EcellIDX = ismember(1:PopNum,Ecells);
Icells = EPopNum+1:PopNum;      IcellIDX = 1:PopNum > EPopNum;  %IcellIDX = ismember(1:PopNum,Icells);
...
if length(PopParams.E_L) == 2 
    E_L = [PopParams.E_L(1).*ones(EPopNum,1);PopParams.E_L(2).*ones(IPopNum,1)];
else
    E_L = PopParams.E_L.*ones(PopNum,1);
end

if length(PopParams.g_L) == 2 
    g_L = [PopParams.g_L(1).*ones(EPopNum,1);PopParams.g_L(2).*ones(IPopNum,1)];
else
    g_L = PopParams.g_L.*ones(PopNum,1);
end

if length(PopParams.C) == 2 
    C = [PopParams.C(1).*ones(EPopNum,1);PopParams.C(2).*ones(IPopNum,1)];
else
    C = PopParams.C.*ones(PopNum,1);
end

if length(PopParams.I_e) == 2 
    I_e = [PopParams.I_e(1).*ones(EPopNum,1);PopParams.I_e(2).*ones(IPopNum,1)];
else
    I_e = PopParams.I_e.*ones(PopNum,1);
end

if length(PopParams.V_th) == 2 
    V_th = [PopParams.V_th(1).*ones(EPopNum,1);PopParams.V_th(2).*ones(IPopNum,1)];
else
    V_th = PopParams.V_th.*ones(PopNum,1);
end

if length(PopParams.V_reset) == 2 
    V_reset = [PopParams.V_reset(1).*ones(EPopNum,1);PopParams.V_reset(2).*ones(IPopNum,1)];
else
    V_reset = PopParams.V_reset.*ones(PopNum,1);
end

if length(PopParams.t_ref) == 2 
    t_ref = [PopParams.t_ref(1).*ones(EPopNum,1);PopParams.t_ref(2).*ones(IPopNum,1)];
else
    t_ref = PopParams.t_ref.*ones(PopNum,1);
end

if length(PopParams.sigma) == 2 
    sigma = [PopParams.sigma(1).*ones(EPopNum,1);PopParams.sigma(2).*ones(IPopNum,1)];
else
    sigma = PopParams.sigma.*ones(PopNum,1);
end

if length(PopParams.theta) == 2 
    theta = [PopParams.theta(1).*ones(EPopNum,1);PopParams.theta(2).*ones(IPopNum,1)];
else
    theta = PopParams.theta.*ones(PopNum,1);
end

if length(PopParams.E_w) == 2 
    E_w = [PopParams.E_w(1).*ones(EPopNum,1);PopParams.E_w(2).*ones(IPopNum,1)];
else
    E_w = PopParams.E_w.*ones(PopNum,1);
end

if length(PopParams.b_w) == 2
    b_w = [PopParams.b_w(1).*ones(EPopNum,1);PopParams.b_w(2).*ones(IPopNum,1)];
else
    b_w = PopParams.b_w.*ones(PopNum,1);
end

if length(PopParams.delta_T) == 2 
    delta_T = [PopParams.delta_T(1).*ones(EPopNum,1);PopParams.delta_T(2).*ones(IPopNum,1)];
else
    delta_T = PopParams.delta_T.*ones(PopNum,1);
end

if length(PopParams.gwnorm) == 2 
    gwnorm = [PopParams.gwnorm(1).*ones(EPopNum,1);PopParams.gwnorm(2).*ones(IPopNum,1)];
else
    gwnorm = PopParams.gwnorm.*ones(PopNum,1);
end

if length(PopParams.w_r) == 2 
    w_r = [PopParams.w_r(1).*ones(EPopNum,1);PopParams.w_r(2).*ones(IPopNum,1)];
else
    w_r = PopParams.w_r.*ones(PopNum,1);
end

if length(PopParams.b) == 2 
    b = [PopParams.E_L(1).*ones(EPopNum,1);PopParams.b(2).*ones(IPopNum,1)];
else
    b = PopParams.b.*ones(PopNum,1);
end

if length(PopParams.E_e) == 2 
    E_e = [PopParams.E_e(1).*ones(EPopNum,1);PopParams.E_e(2).*ones(IPopNum,1)];
else
...
if length(PopParams.E_L) == 2 
    tau_s = [PopParams.tau_s(1).*ones(EPopNum,1);PopParams.tau_s(2).*ones(IPopNum,1)];
else
...
for tt=1:SimTimeLength
    ...
    if mod(timecounter,save_dt)==0 && timecounter>=0
        if recordVALs(tt)
         ...
         SimValues.Input(:,savecounter)           = I_e + X_t;
